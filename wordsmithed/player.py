...
import time, board, tile, bag, aistats, heuristic
...
class Player:
	...
	def executeTurn(self, isFirstTurn):
		...
		seeds = []
		if isFirstTurn:
			seeds.append((board.Board.START_POSITION[0],board.Board.START_POSITION[1])) #The seed has to be (7,7) for the first turn
		
		else:	
			#find all empty adjacent squares and add them to the list	
			for x in range(self.theBoard.GRID_SIZE):
				for y in range(self.theBoard.GRID_SIZE):
					if self.theBoard.squares[x][y][0] != None:
						...
						if y > 0 and self.theBoard.squares[x][y-1][0] == None:
							if not (x, y-1) in seeds:
								seeds.append((x, y-1))
						#DOWN
						if y < self.theBoard.GRID_SIZE-1 and self.theBoard.squares[x][y+1][0] == None:
							if not (x, y+1) in seeds:
								seeds.append((x, y+1))
						#LEFT
						if x > 0 and self.theBoard.squares[x-1][y][0] == None:
							if not (x-1, y) in seeds:
								seeds.append((x-1, y))
						#RIGHT
						if x < self.theBoard.GRID_SIZE-1 and self.theBoard.squares[x-1][y][0] == None:
							if not (x+1, y) in seeds:
								seeds.append((x+1, y))
		...
		for (x, y) in seeds:
			...
			for lo in range(0, len(self.tray)):
				for hi in range(0, len(self.tray)-lo):
					...
					#Build right
					xPos, yPos = x+1, y
					while xPos < self.theBoard.GRID_SIZE-1 and (hiCount < hi or self.theBoard.squares[xPos][yPos][0] != None):
						hiCount += 1
					...
					#Build down
					xPos, yPos = x, y+1
					while yPos < self.theBoard.GRID_SIZE-1 and (hiCount < hi or self.theBoard.squares[xPos][yPos][0] != None):
						hiCount += 1
