...
var socket = new io.connect("http://lexycross.com:80");
...
var game = {
    ...
    updateGame: function (data) {
        ...
        $('#score .score-info').remove();

        $('#currentTurn').html('<span>' + turnName + '\'s move</span>');
        var playerId; //who played what
        for (var i in data.players) {
            var _player = data.players[i];
            $('#scoreboard').append("<div class='pull-left text-info'><strong>" + _player.name + "</strong></div><div class='pull-right'>" + _player.score + "</div><div class='clearfix'/>");
        }
        

        $('#scoreboard').append("<div class='pull-left'>Tiles Left</div><div class='pull-right'>" + data.tilesleft + "</div><br/><div class='clearfix'></div>");
        ...
        for (var i in data.scoreboard) {
            ...
            for (var j in data.scoreboard[i].scoreInfo) {
                ...
                $('#score').append("<div class='score-info'>" + _player.name + ':' + _score.word + ' for ' + _score.total + "</div>");
            }
        }
        $('#score').scrollTop($('#score div:last').offset().top);
